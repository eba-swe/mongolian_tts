...
from datasets.commonVoiceMN import commonVoiceMN


parser = argparse.ArgumentParser(
    description=__doc__, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("--dataset", required=True,
                    choices=['ljspeech', 'mbspeech'], help='dataset name')
args = parser.parse_args()


dataset_name = 'MBSpeech-1.0'
datasets_path = os.path.join(os.path.dirname(
...
if os.path.isdir(dataset_path) and False:
	...
	for bible_book_name in bible_books:
		...
		bible_book_file_path = os.path.join(
			datasets_path, bible_book_file_name)
		if not os.path.isfile(bible_book_file_path):
			url = "https://s3.us-east-2.amazonaws.com/bible.davarpartners.com/Mongolian/" + \
...
dataset_csv_file_path = os.path.join(
	datasets_path, '%s-csv.zip' % dataset_name)
dataset_csv_extracted_path = os.path.join(
	datasets_path, '%s-csv' % dataset_name)
...
def _convert_mp3_to_wav(book_name, book_nr):
	...
	while True:
		try:
			i = 0
			chapter_csv_file_name = os.path.join(
				dataset_csv_extracted_path, "%s_%02d.csv" % (book_name, chapter))
			...
			for index, row in df.iterrows():
				...
				metadata_csv_writer.writerow(
					[fn, sentence, sentence])  # same format as LJSpeech
				wav = samples[start:end]
				# use same sample rate as LJSpeech
				wav = librosa.resample(wav, sample_rate, hp.sr)
				librosa.output.write_wav(os.path.join(
					wavs_path, fn + ".wav"), wav, hp.sr)
